...
_World Wide Web Wanderer_ in June 1993 to 100,000 in January 1996, @MGray.
...
In 1996, two PhD students at Stanford University began doing research on search
engines. They had two main complaints concerning the search engines that were
...
results for the first time in, @brin1998anatomy.
...
To address this, Page and Brin setup on the idea of using a probability
...
lets use a $n \times n$ matrix, $A$, where the column $j$ of the $i^{th}$ row is one
of the website $j$ points (has a link) to $i$.
The importance of a page can then be written as:
...
That problem is an eigenvalue and eigenvector problem. All you have to do is
take the eigenvector corresponding to the highest eigenvalue of the matrix $A$.
Then, the most important website is the one with the largest entry in that
vector.
...
user clicking on random links). Also, the rank of the pages that links to our
page are normalized by the number of links leaving that page.
...
the right weight is empirical), the score given by this method is called the IR
score.
